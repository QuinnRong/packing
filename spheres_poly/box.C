...
{
  double rand_num;                       // random number between -0.5 and 0.5
  ...
  rand_num = (double)rand() / (double)RAND_MAX - 0.5;
  if(rand_num < 0) 
    {
      rand_num = -rand_num;
      dv = -dv;
  ...
  while(fabs(p) < rand_num) // integrate until the integral equals rand
    {
...
void box::WriteConfiguration(const char* wconfigfile)
{
 ...
 // output << "\n";
  ...
  printf("radius1 = %f, radius2 = %f\n", s[1].r, s[1].r*bidispersityratio);
  output << "\n" << N << " atoms\n\n";
  output << "2 atom types\n\n";
  output << "0 1 xlo xhi\n";
  output << "0 1 ylo yhi\n";
 ...
 for (int i=0; i<N*bidispersityfraction; i++)  // output radius and positions
   {
     output << i + 1 << " " << 1 << " ";
     for (int k=0; k<DIM; k++)
 ...
 for (int i=N*bidispersityfraction; i<N; i++)  // output radius and positions
   {
     output << i + 1 << " " << 2 << " ";
     for (int k=0; k<DIM; k++)
       output << std::setprecision(16) << s[i].x[k] << " ";
